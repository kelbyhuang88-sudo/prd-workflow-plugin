...
```
需求文档/
├── 费用审批需求文档_v1.md    ← 第一次编写
├── 费用审批需求文档_v2.md    ← 二期迭代/大幅修改
└── 费用审批需求文档_v3.md    ← 三期迭代
```

**命名格式：** `{需求名称}需求文档_v{版本号}.md`

### 同名文件处理

**检测到已存在PRD文档时：**

> "检测到已存在 `{需求名称}需求文档_v{N}.md`，请选择：
> 
> | 选项 | 说明 |
> |------|------|
> | **A. 创建新版本** | 生成 `{需求名称}需求文档_v{N+1}.md`，保留历史版本 |
> | **B. 覆盖原文件** | 替换原有 `{需求名称}需求文档_v{N}.md` |
> | **C. 对比后决定** | 先展示新旧差异，再决定 |
> 
> 请选择："
...
| 用户选择 | 处理 |
|----------|------|
| A（创建新版本） | 生成新版本文件，版本号递增 |
| B（覆盖原文件） | 使用Edit工具更新原有文件 |
| C（对比后决定） | 展示diff，等待用户确认 |

**默认行为：** 首次编写时生成 `v1`，后续默认询问用户选择。
